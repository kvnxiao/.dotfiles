...
The helper narrows every scope before Codex sees it, and every remaining region is an editable block the model addresses by ID. A prose file includes the paragraphs and list items its diff touched; a source file includes only its touched line-comment runs and is reported as `no-changed-comment-lines` when none changed; a file with no comment syntax, such as JSON, a lockfile, or a sqitch plan, is reported as `non-prose-file` and is not audited; a draft excludes its blank lines and its trailer lines. In a prose target, fenced code, tables, frontmatter, blockquotes, thematic breaks, and headings in either syntax are context. A prose file whose name starts with `AGENTS`, `CLAUDE`, or `SKILL` (`CLAUDE.local.md` included), or one in a `skills`, `output-styles`, `commands`, or `prompts` directory, is audited as an `instruction-file`: imperative rules, indicative premises, bold lead-ins kept verbatim. Every other prose file is `documentation`. In a source file, a shebang, a lint pragma, and a list inside a comment run break the run and stay context. The audit therefore covers reflowable prose only. Report a narrowed or dropped target to the user rather than working around it.
...
- Status 4 is a clean `NO_CHANGES` result: Codex returned `no_changes`, or every edit reproduced its source block and none was dropped. Report a clean prose pass and apply nothing.
...
- A `PARTIAL` line names a target whose patch the helper built from the surviving edits, and lists each dropped block with its reason: an unknown or repeated block ID, a block that no longer fits the file, a replacement carrying a line separator, an empty replacement outside a `code-comment` target, a replacement that loses a code span, bold span, path, filename, version, flag, number, or identifier the source block contains, a replacement that splits a sentence, a replacement past the target's wrap width, or a line copied from another target or already present in this target. The patch beside it is complete as written, so assess it the way you assess any other target. Report each dropped block as unaudited prose, and do not relaunch the bundle to retry it.
...
- An `UNCHANGED` line names a target whose surviving edits reproduced its source block. It needs no action and appears with status 0 or 4; when the same target also lost edits, its drop line reads `PARTIAL`.
...
The helper rejects missing inputs, stale targets, changed repository inventories, incomplete target acknowledgments, unknown or tool events, malformed edit fields, and an override naming no prepared target. It drops any single edit that names an unknown or repeated block, carries a line separator, empties a block outside a `code-comment` target, loses a code span, bold span, path, filename, version, flag, number, or identifier its source block contains, splits a sentence, exceeds the target's wrap width, copies a line from another target, or duplicates a line the target already contains. A target retains the edits that survive; a target whose surviving edits reproduce its source is reported as `UNCHANGED`. The helper controls patch paths, hunk positions, counts, line endings, artifact kinds, block boundaries, line wrapping, list and comment markers, and file lifecycle boundaries.
