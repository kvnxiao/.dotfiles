...
- **Demonstrative noun-echo:** Flag a sentence ending on a noun where the next sentence opens `That <same noun>`, such as `... without the schema file. That file is absent from fresh checkouts.` The echo re-announces a referent the reader has not lost. Fold the second sentence's modifier into the noun's first appearance with a restrictive relative or a prepositional phrase. Keep the split where the second sentence carries an independent operational rule rather than a modifier.
...
- **Meta-documentation:** Flag prose whose subject is the documentation rather than the code: why a symbol is not a cross-reference link, why a fact sits in this comment, what a caller `discovers`, `notices`, or `is told`. Replace it with the constraint in one clause, or delete it where the code already carries the fact. Keep a note that stops a maintainer from undoing a deliberate choice.
