...
- **Reuse:** Search nearby code and shared modules for an existing implementation. Name the helper
  ...
  unrelated code or adding a new abstraction.
...
For each proposal, compare return values, errors, side effects, evaluation order, resource lifetime,
synchronization, and compatibility where applicable. A faster implementation that changes these
contracts is not a behavior-preserving cleanup. When equivalence depends on an unverified
assumption, report the assumption separately instead of recommending the change as established.
...
- **Confirmed:** The proposed replacement has a concrete benefit and preserves behavior, as
  established by inspection or observed execution.
- **Unresolved:** The benefit or equivalence depends on an unverified assumption. State the missing
